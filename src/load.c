...
int
load_file (const floc *flocp, void *f, int noerror)
{
   ...
   * Return -1 to ensure make does not rebuilt this shared object.
   * A rebuilt, if required, took place when this object was loaded.  */
  {
    struct file *ff = lookup_file (ldname);
    if (ff && ff->loaded)
      return -1;
  }
  ...
  if (r > 0)
    {
      ...
      loadlen = strlen (loaded);
      newval = alloca (loadlen + strlen (ldname) + 2);
...
int
load_file (const floc *flocp, void *f UNUSED, int noerror)
{
  if (! noerror)
    O (fatal, flocp,
