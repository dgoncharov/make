      ...
      if (!two_colon)
        {
           ...
           * There is no need to set is_explicit in the case of double colon
           * below, because an implicit double colon rule only applies when the
           * prerequisite exists. A prerequisite which exists is not
           * intermediate anyway. */
          f->is_explicit = 1;
        }
      else
